...
Return only the JSON dictionary, nothing else."""
    ...
    try:
        ...
        # Extra safety — remove any nested values
        clean_metadata = {}
...
def vectorstore_creation(all_child_chunks):
    vectorstore=Chroma(embedding_function=OpenAIEmbeddings(model="text-embedding-3-small"),persist_directory="./chroma_db")
    vectorstore.add_documents(all_child_chunks)
    return vectorstore

@observe()
def hybrid_search_rerank(query,vectorstore,all_child_chunks):
...
@observe()
def main():
    if (os.path.exists("./chroma_db") and os.path.exists("parent_chunk.json") and os.path.exists("child_chunks.pkl")):
        vectorstore = Chroma(
        ...
        all_child_chunks = [Document(page_content=i, metadata=j) for i, j in zip(result["documents"], result["metadatas"])]

    else:
        # check if child_chunks.pkl exists but chroma_db does not
        if os.path.exists("child_chunks.pkl") and os.path.exists("parent_chunk.json"):
    ...
    results = hybrid_search_rerank(query, vectorstore, all_child_chunks)
